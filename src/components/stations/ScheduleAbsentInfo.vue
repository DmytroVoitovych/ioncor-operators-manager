...
const absentInfo = computed(() => {
  ...
  else if (props.absentAmount < 0)
    return `There’s a surplus worker because there are more workers than stations: ${amountInfo}`;
  else if (props.absentAmount === 1)
    return `You lack one operator ${amountInfo} One slot will be excluded from the station 200`;
  else
    return `You don't have enough operators ${amountInfo} When generating the slot will be filled with "Extra"`;
});
