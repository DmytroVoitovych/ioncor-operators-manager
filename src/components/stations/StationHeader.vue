<template>
  <div class="card-header">
    <h2 class="text-preset-6">
      Generated schedule for <span class="text-preset-6-mono">{{ now }}</span>
    </h2>
    <el-date-picker
      v-model="now"
      type="date"
...
const setDisabledDate = (time: Date) => {
  ...
  return isDateAllowed || !getSnapshotMap.value.keys()?.some((key) => key.includes(formatedDate));
};
