...
const getSuitableWorkersForReplacement = (
  ...
  const idealWorker = shuffle(workersWhoKnowBothStations.filter(
    (e) =>
...
const findWorkerAsLastResort = (
  ...
  const giveNotLastVisited = (e: StationNumber) => e !== "130" && worker.station_history.at(-1)?.station !== e;
  ...
  const checkStForPossibleAndKnown = (e: Operator) =>
    e.known_stations.includes(stationId) && possibleStations.includes(e.current_station) && e.station_history.at(-1)?.station !== stationId;

  const newReplaceWith =
    availableWorkers.find(
      (e) => checkStForPossibleAndKnown(e) && !e.visited_stations?.includes(stationId) && e.station_history.at(-1)?.station !== stationId,
    ) ||
...
const findWorkerForSwap = (
 ...
 const worker = shuffle(availableWorkers
    .filter(
      (e) =>
